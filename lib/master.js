...
Master.prototype._fork = function (appId, options, cb) {
  ...
  try {
    ...
    return cb(new Error('master._fork() error , App Dir: ' + options.dir + ' , enter file not found, Options: ' + JSON.stringify(options) + 'please check app\'s package.json `main` property'));
  }
  /*
...
Master.prototype.getAppProcessNumFromPkg = function (appDir) {
  let processorNum = null;
  let pkg = {};
  try {
  ...
  return isNaN(processorNum) ? null : processorNum;
};
/**
 * 挂载app
...
Master.prototype.$mount = function (appId, options, cb) {
  ...
  if (!options.dir || !appId) {
    return cb(new Error('Error APPConfig appId: ' + appId + ', options: ' + JSON.stringify(options)));
  }
  let appInfo = utils.parseAppId(appId);
  let pkgProcessNum = this.getAppProcessNumFromPkg(options.dir);
  let privateConfig = this.getAppConfig(appInfo.name);
  this._fork(appId, {
    config: privateConfig,
    dir: options.dir,
    processorNum: options.processorNum || privateConfig.processorNum || pkgProcessNum || 1
  }, (err) => {
    if (!err && !this.appSession.get(appId) && !options.noPersistent) {
...
Master.prototype.$reload = function (appId, cb) {
   ...
   * reload config
   */
  let pkgProcessNum = this.getAppProcessNumFromPkg(child.options.dir);
  child.options.config = this.getAppConfig(child.options.name);

  child.options.processorNum = child.options.config.processorNum || pkgProcessNum || child.options.processorNum;
