...
Master.prototype.init = function (done) {
  ...
  function shutdown(sig, err) {
    ...
    log.error(FLAG_MASTER, `server received ${sig}, will shutdown.`, err ? (err.stack || err) : 'unknow error');
    self.exit(function () {
      process.exit(1);
    });
  }
...
/**
 * health check
 * 如果有应用在发布中，则 flagHealthCheck = false, 会返回 "hoenycomb-server offline" 错误信息
 * 如果某一个在线的children, 其配置的健康检查接口不正常，则返回 appid和错误信息
 */
...
// 存活检查，只探测在线状态的app，如果状态假死则其 health接口探测失败，能及时发现
Master.prototype.$livenessCheck = function (cb) {
  let totalApps = 0;
  let count = 0;
  const children = this.children;
  let errs = [];
  ...
  for (let i = 0; i < totalApps; i++) {
    let appId = appIds[i];
    if (appId === APPID_PROXY || appId === APPID_ADMIN || children[appId].status !== 'online') {
      totalApps--;
      continue;
    }
    children[appId].healthCheck(done);
...
Master.prototype.$appExitCount = function (sinceTime, cb) {
  const children = this.children;
  let count = 0;
  for (let appId in children) {
    let cc = children[appId];
    let ll = cc.errorExitRecord.length;
    for (let i = 0; i < ll; i++) {
      let t = cc.errorExitRecord[i];
      if (t >= sinceTime) {
        count += (ll - i);
      }
    }
...
Master.prototype.$healthCheckEnable = function (cb) {
  cb(null, this.flagHealthCheck);
};
...
Master.prototype.$reload = function (appId, cb) {
  ...
  if (child.options.config.honeycomb.killCurrentBeforeMount) {
    ...
    // $mount里面有流量控制处理，先offline切断流量, 再关闭app
    // $mount里先杀死当前app的历史版本，再启动版本
    this.$mount(appId, child.options, cb);
    return;
  }
