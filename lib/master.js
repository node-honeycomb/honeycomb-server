...
Master.prototype.$mount = function (appId, options, cb) {
  ...
  if (privateConfig.honeycomb) {
    if (privateConfig.honeycomb.processorNum !== undefined) {
      processorNum = Number(privateConfig.honeycomb.processorNum);
    }
    if (privateConfig.honeycomb.order != undefined) {
      order = privateConfig.honeycomb.order;
    }
  }
  if (appOptions.job) {
    appOptions.job.name = pkg.name;
    appOptions.job.version = pkg.version || '0.0.1';
    appOptions.job.buildNum = pkg.build || 1;
  }
  ...
  if (isNaN(processorNum) || processorNum <= 0) {
    processorNum = 1;
  }
