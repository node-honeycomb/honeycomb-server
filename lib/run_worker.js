...
process.on('error', function (err) {
  ...
  console.error(FLAG_RUNTIME, `thread_worker_error, code:${err.code || ''} msg:${message || ''}, exit`);
...
appReadyTimeoutCheck = setTimeout(function () {
  let err = new Error(`thread_worker:${appId} timeout, try to start app faild, app.run() timeout`);
...
if (app && app.run) {
  ...
  app.run(function (err, config) {
    ...
    // eslint-disable-next-line
    console.log(FLAG_RUNTIME, `thread_worker_ready , bind: ${bind || 'default'}, router: ${config.router}, target: ${target}`);
    /**
     * config
     *   appId
  ...
  let e = new Error(`thread_worker_missing_run_method : ${appId} , should export app.run(callback) method ${file}`);
  e.exitCode = 14;
  process.emit('error', e);
}
