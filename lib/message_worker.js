 ...
 * 接口及使用方法：
 *
 * 1. 首先这是个单例模式， require('lib/worker_message.js')即可得到实例
 *
 * 2. 发送消息
 *
 *  /** 常规事件机制 **
...
class Message extends events.EventEmitter {
   ...
   * [receive description]
   * @param  {Object} msg
   * @param  {Worker} proc    worker instance
   * @param  {Object} bindObj action 实际执行对象
   */
  receive(msg, proc, bindObj) {
    ...
    function callback(err, data) {
      ...
      self.send({
        ...
        data: data,
        target: proc,
        requestAction: action // 追踪一下是什么动作的callback
      });
    }
    ...
    if (this.queues[id]) {
      ...
      // do nothing here
      console.log('process message already timeout', msg); // eslint-disable-line
      return true;
    } else if (msg.group) {
      return true;
    } else {
      ...
      if (actionName.startsWith('$')) {
        ...
        // check if evtTarget have bind event
        if (evtTarget.listenerCount(actionName)) {
          if (msg.data) {
            evtTarget.emit(actionName, msg.data, callback);
          } else {
            evtTarget.emit(actionName, callback);
          }
        } else {
          callback(new Error('unlistened event:' + actionName));
        }
      }
      return true;
    }
