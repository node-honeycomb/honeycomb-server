  ...
  _fork(exec, args, env) {
    ...
    if (worker.stdout && worker.stderr) {
      ...
      worker.stdout.on('data', (chunk) => {
        console.log(log.getTime(), appId, '#' + pid, chunk.toString().trimRight()); // eslint-disable-line
      });
      worker.stderr.on('data', (chunk) => {
        console.error(log.getTime(), appId, '#' + pid, 'ERROR', chunk.toString().trimRight()); // eslint-disable-line
      });
    }
    return worker;
  }
