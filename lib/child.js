...
class Child extends EventEmitter {
  ...
  _fork(exec, args, env) {
    ...
    if (worker.stdout && worker.stderr) {
      ...
      if (this.options.appRoot && !this.options.appRoot.match(/(.*admin$)|(.*proxy$)/g)) {
        config.logs.nodejsStd = config.logs.nodejsStd || {level: 'INFO'};
      ...
      if (!logStd) {
        worker.stdout.on('data', (chunk) => {
          // eslint-disable-next-line
          console.log(`${log.getTime()} ${appId} '#' ${pid}`, chunk.toString().trimRight());
        });
        worker.stderr.on('data', (chunk) => {
          // eslint-disable-next-line
          console.error(`${log.getTime()} ${appId} # ${pid} ERROR`, chunk.toString().trimRight());
        });
      } else {
        worker.stdout.on('data', (chunk) => {
  ...
  _kill(worker, cb) {
    ...
    timeout = setTimeout(() => {
      log.warn('app exit timeout, now force killing');
      end();
