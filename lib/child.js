...
class Child extends EventEmitter {
  ...
  _fork(exec, args, env) {
    ...
    if (worker.stdout && worker.stderr) {
      ...
      let logStd = console.log;
      let prefixed = true;
      if (this.options.appRoot && !this.options.appRoot.match(/(.*admin$)|(.*proxy$)/g)) {
        config.logs.nodejsStd = {level: 'DEBUG'};
        let logDir = path.join(config.logsRoot || this.options.appRoot, utils.parseAppId(this.options.appId).name);
        xfs.mkdir(logDir);
        config.logs.nodejsStd.file = path.join(logDir, 'nodejs_std%year%-%month%-%day%.log');
        try {
          logStd = LiteLog.create({
            nodejs_std: config.logs.nodejsStd
          });
          logStd = logStd.get('nodejs_std');
          logStd = logStd.debug.bind(logStd);
          prefixed = false;
        } catch (e) {
          // eslint-disable-next-line
          logStd = console.log;
      ...
      logStd(prefixed ? (`${log.getTime()} ${appId} # ${pid} ERROR`) : '', 'aaaaa');
      worker.stdout.on('data', (chunk) => {
        logStd(prefixed ? (`${log.getTime()} ${appId} '#' ${pid}`) : '', chunk.toString().trimRight());
      });
      worker.stderr.on('data', (chunk) => {
        logStd(prefixed ? (`${log.getTime()} ${appId} # ${pid} ERROR`) : '', chunk.toString().trimRight());
      });
    }
    return worker;
  }
  ...
  _create() {
    ...
    childProc.on('exit', function (code, signal) {
      ...
      log.warn(FLAG_CHILD, 'app exit:', options.appId, 'with signal:', signal, 'code:', code, 'status:', self.status);
