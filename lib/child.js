...
class Child extends EventEmitter {
  constructor(appId) {
    ...
    this.options = {
      ...
      service: null // Object exec 类型才有配置
    };
    /**
     * worker list {Object}
     */
  ...
  _createChild(sockOrPort) {
    ...
    function checkExecService() {
      count++;
      let client = net.connect(targetPort || targetSock, () => {
        childProc.ready = true;
        client.removeAllListeners();
        client.destroy();
        log.info(FLAG_CHILD, `${options.appId} fork child successfully, app status: ${self.status}`);
      ...
      client.on('error', () => {
        ...
        if (count > 1200) {
          ...
          setTimeout(checkExecService, 400);
        }
      });
    }
