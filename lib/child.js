...
class Child extends EventEmitter {
  ...
  /**
   * reload workers, only online status can do this action
   */
  reload(cb) {
    ...
    this.status = 'reload';
    let oldWorkers = Object.keys(this.workers);
    // reset workers
    this.oldWorkers = this.workers;
    this.workers = {};
    // re-assign new sock files
    this.initTargetSocks(oldWorkers.length);

    this.once('reloaded', (config) => {
      config.reload = true;
      /**
       * tell master that new workers is ready,
       * master will start reload nginx to let
    ...
    log.info(FLAG_CHILD, `app_start_reload : ${this.appId} , old workers:`, oldWorkers);
  ...
  _create() {
    ...
    let argv = [options.appId].concat(process.argv.slice(2));
    ...
    childProc.on('exit', function (code, signal) {
      ...
      // log.warn('app exit:', options.appId, 'with signal:', signal, 'code:', code);
  ...
  stop(cb) {
    if (this.status === 'reload') {
      return cb && cb(new Error('server is executing command now, please waitting for awhile'));
    }
