  ...
  _fork(exec, args, env) {
    ...
    if (worker.stdout && worker.stderr) {
      ...
      if (this.options.appRoot && !this.options.appRoot.match(/(.*admin$)|(.*proxy$)/g)) {
        config.logs.nodejsStd = config.logs.nodejsStd || {};
        let logDir = path.join(config.logs.nodejsStd.fileDir || this.options.appRoot, this.options.appId.split('_')[0]);
        xfs.mkdir(logDir);
      ...
      worker.stdout.on('data', (chunk) => {
        logStd(log.getTime(), appId, '#' + pid, chunk.toString().trimRight()); // eslint-disable-line
      });
      worker.stderr.on('data', (chunk) => {
        logStd(log.getTime(), appId, '#' + pid, 'ERROR', chunk.toString().trimRight()); // eslint-disable-line
      });
    }
    return worker;
  }
