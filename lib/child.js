...
const JAVA_PROGRAM = 'java';
const CGROUP_MEMORY_LIMIT_FILE = '/sys/fs/cgroup/memory/memory.limit_in_bytes';
const MEMORY_SIZE_BOUND = 17179869184; // 16G
...
function getCGroupMemoryLimit() {
  if (!fs.existsSync(CGROUP_MEMORY_LIMIT_FILE)) {
    return 0;
  }

  try {
    return parseInt(fs.readFileSync(CGROUP_MEMORY_LIMIT_FILE));
  } catch (error) {
    return 0;
  }
}
...
class Child extends EventEmitter {
  constructor(appId) {
    ...
    this.memoryLimit = getCGroupMemoryLimit();
    this.heapNewSize = Math.ceil(this.memoryLimit * 0.25 / 1024 / 1024); // 单位：M
    this.heapInitSize = Math.ceil(this.memoryLimit * 0.5 / 1024 / 1024); // 单位：M
    this.heapMaxSize = Math.ceil(this.memoryLimit * 0.75 / 1024 / 1024); // 单位：M
    ...
    this.on('worker_ready', (appCfg) => {
      ...
      if (this.checkWorkerReady() && this.status !== 'online') {
        ...
        appCfg = _.cloneDeep(this.options);
  ...
  _adjustJavaArgv(str) {
    ...
    if (this.memoryLimit == 0 ||
        this.memoryLimit >= MEMORY_SIZE_BOUND) {
      return str;
    }
    let param = str.substring(0, 4);
    let sz = utils.parseMemorySize(str.substring(4));
    if (param == '-Xmn' && sz >= this.memoryLimit * 0.5) {
      let newSize = '-Xmn' + this.heapNewSize + 'M';
      log.info(FLAG_CHILD, `Adjust -Xmn from ${str} to ${newSize}`);
      return newSize;
    } else if (param == '-Xms' && sz >= this.memoryLimit * 0.5) {
      let initSize = '-Xms' + this.heapInitSize + 'M';
      log.info(FLAG_CHILD, `Adjust -Xms from ${str} to ${initSize}`);
      return initSize;
    } else if (param == '-Xmx' && sz >= this.memoryLimit) {
      let maxSize = '-Xmx' + this.heapMaxSize + 'M';
      log.info(FLAG_CHILD, `Adjust -Xmx from ${str} to ${maxSize}`);
      return maxSize;
    } else {
      return str;
    }
  ...
  _createChildProc(sockOrPort) {
    ...
    if (options.job) {
      ...
      // 针对java类型的应用，如果设置的-Xmx等参数过大，先进行自动调整
      if (path.basename(options.service.exec) == JAVA_PROGRAM) {
        argv.forEach((v, i, a) => {
          a[i] = this._adjustJavaArgv(v + '');
        });
      }
  ...
  stop(cb) {
    ...
    log.warn(FLAG_CHILD, `app_shutting_down : ${this.appId}, pids: ${Object.keys(this.workers)}`);
    // clearTimeout(this.readyTimeoutId);
    // this.readyTimeoutId = null;
    // clear retry timeout
    this.retryTimeoutId.forEach((timeout) => {
  ...
  _kill(worker, cb) {
    ...
    function end() {
      ...
      if (worker.exitCode !== null) {
        log.warn(`worker already killed, appId: ${appId}, pid: ${worker.pid}`);
        cb();
      } else {
        log.warn(`worker force kill, appId: ${appId}, send -9 signal to worker process`);
        let ok = worker.kill('SIGKILL'); // -9
        log.warn(`worker force kill, appId: ${appId} kill -9 return:`, ok);
        killCount++;
        if (killCount > 30) {
          let err = new Error(`app_worker_force_kill_failed, appId: ${appId}, please retry`);
    ...
    timeout = setTimeout(() => {
      log.warn(FLAG_CHILD, `app_exit_timeout: ${appId}, now force killing`);
      end();
