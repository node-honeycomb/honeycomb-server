...
const M = 1024 * 1024;
const log = Log.create({appUsage: config.logs.appUsage});
    ...
    interval = setInterval(() => {
      message.send({
        ...
        tasks.push((callback) => {
          ...
          // Error: spawn ENOMEM
          util.sysMemUsage((err, percentage) => {
            if (err) {
              log.warn('get system memory usage failed:', err.message);
              return callback();
            }
            // os.loadavg() 返回一个数组，分别是1，5，15分钟的系统负载平均值，这里取1分钟的值
            let sysLoadavg = os.loadavg()[0].toFixed(1);
            let sysMem = Math.floor(percentage * 100);
