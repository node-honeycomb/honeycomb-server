...
exports.reloadApps = function (req, res) {
  ...
  utils.callremote(path, {method: 'POST', ips: ips, series: true}, function (err, results) {
    res.json(resultsWrap(err, results));
  });
};
...
exports.startApps = function (req, res) {
  ...
  utils.callremote(path, {method: 'POST', ips: ips}, function (err, results) {
    res.json(resultsWrap(err, results));
  });
};
  ...
  async.waterfall([
    ...
    function (file, callback) {
      ...
      utils.callremote(apiPath, {
        ...
        timeout: config.admin.serverTimeout / 2,
        series: config.seriesPublish
      }, callback);
    }
  ], function (err, data) {
