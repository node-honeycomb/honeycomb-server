...
exports.queryLog = function (req, res) {
  ...
  const file = req.query.file || req.query.fileName;
  const startTime = req.query.startTime;
  const logFile = path.join(logRoot, file);
  const filter = req.query.filter || req.query.filterString;
    ...
    if (startTime || filter) {
      ...
      xs.slice(-lines, function (err, data) {
        if (err) {
          ...
          return res.json({code: 'ERROR', message: `日志查询失败: ${err.message}`});
        }
        let results = [];
        data.forEach((v) => {
          let line = v.toString();
...
exports.queryLogFiles = function (req, res) {
  function scanLogFiles(dir) {
    ...
    try {
      ...
      if (e.code !== 'EACCES') {
        log.error('read log dir failed: ', e);
      }
      return;
    }
