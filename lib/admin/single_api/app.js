...
exports.publishApp = function (req, res, next) {
  ...
  async.waterfall([
    ...
    function untar(callback) {
      utils.exec('tar zxfm ' + fileName, {cwd: config.appsRoot, maxBuffer: 128 * 1024 * 1024}, callback);
    },
    function hook(rows, callback) {
    ...
    function checksum(rows, callback) {
      if (!config.appCheckSumSecret) {
        callback(null);
      }
      let checksum = path.join(config.appsRoot,'checksum.txt');
      if (!fs.existsSync(checksum)) {
        utils.exec(`rm -rf ${fileName} ${appId}`, {cwd: config.appsRoot}, (err) => {
          callback(err || new Error('app checksum failed, checksum.txt not found'));
        });
        return; 
      }
