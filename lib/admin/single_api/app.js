...
exports.publishApp = function (req, res, next) {
  ...
  async.waterfall([
    ...
    function (fileName, callback) {
      utils.exec('tar zxf ' + fileName, {cwd: config.appsRoot, maxBuffer: 64 * 1024 * 1024}, callback);
    },
    function (rows, callback) {
      if (nostart) {
        callback(null);
...
exports.setAppConfig = function (req, res, next) {
  ...
  let configFile = getResolvedFile(commonName, appName);
  let cfg = req.body;
...
exports.listDeadProcess = function (req, res, next) {
...
/**
 * @api DELETE /api/single/kill_process/:pid
 */
exports.killDeadProcess = function (req, res, next) {
  let pidsShouldKill = req.params.pid.trim().split(',');
  listProcessors(function (err, deadPids) {
    let deadPidsMap = {};
    deadPids.forEach((proc) => {
      deadPidsMap[proc.pid] = true;
    });
    async.eachSeries(pidsShouldKill, function (pid, done) {
      if (deadPidsMap[pid] >= 0) {
        utils.exec('kill -9 ' + pid, done);
      } else {
        done(new Error('process is not a dead one:' + pid));
      }
    }, function (err) {
      if (err) {
        return next({
