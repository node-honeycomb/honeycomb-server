...
class Proxy extends Events {
  constructor() {
    ...
    this.apps = [];
    this.map = {};
    this.appsBackup = null;
  }
  init(options) {
    if (!options.bind || !options.bind.length) {
      let tmp = {
        ip: options.ip || '*',
   ...
   *   // param > app.config.honeycomb.serverParam
   *   param: {}
   */
  register(app, cb) {
    this.backup();
    ...
    if (!app.reload) {
      log.info(FLAG_PROXY, `receive route change, app: ${app.appId}, type: ${app.type}, bind: ${app.bind}, router: ${app.router}`);
      this.updateRouter(cb);
    } else {
      // 如果是reload, 不触发路由更新下
      // 因为master.$reload中会call proxy.updateRouter() // master.js Line:760
      log.info(FLAG_PROXY, 'receive route change from app reloaded');
      app.reload = false;
    }
  }
  ...
  prepareBind(config, options) {
    ...
    config.bind.forEach(function (v, i, a) {
      ...
      if (/^\d+$/.test(v)) {
        ...
        if (!/^\d+$/.test(v) && !/^[:\da-zA-Z]+:\d+$/.test(v) && !/^(\d+\.){3}\d+:\d+$/.test(v)) {
