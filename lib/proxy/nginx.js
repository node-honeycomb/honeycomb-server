...
class Nginx extends Proxy {
  ...
  initConfig() {
    ...
    ast.statements.forEach((node) => {
      ...
      if (node.key === 'http') {
        ...
        log.error('honeycomb-server::nginx not yet support stream proxy');
      }
    });
      ...
      */
      ...
      switch (app.type) {
        case 'stream':
          ...
          // TODO stream config
          log.error(FLAG_NGINX, 'honeycomb-server::nginx not yet support stream proxy');
          break;
        default:
  ...
  flushConfig(config) {
    ...
    Object.keys(httpConfig.servers).forEach((key) => {
      let cfg = this.genServerConfig(key, httpConfig.servers[key]);
