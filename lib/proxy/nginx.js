...
class Nginx extends Proxy {
          ...
          this.options.bind.forEach((cfg) => {
            ...
            if (!ngConfig.servers[defaultKey]) {
              ...
              if (this.options.healthCheck.router) {
                if (this.options.healthCheck.file) {
                  defaultServerCfg.locations[this.options.healthCheck.router] = {
                    file: this.options.healthCheck.file
                  };
                } else if (this.options.healthCheck.url) {
                  defaultServerCfg.locations[this.options.healthCheck.router] = {
                    url: this.options.healthCheck.url
                  };
                } else {
                  log.error('unknow type of healthCheckConfig');
                }
              }
              ...
              if (app.static) {
                ...
                Object.keys(app.static).forEach((key) => {
                  if (app.static[key].dir) {
                    let obj = {};
                    if (v.param && v.param.location) {
                      _.merge(obj, v.param.location);
                    }
                    if (app.static[key].gziped) {
                      obj.gzip = 'off';
                      obj.add_header = 'Content-Encoding gzip';
                    }
                    obj.alias = app.static[key].dir;
                    let r = path.join(router, '/', key);
                    tmp.locations[r] = obj;
                  }
                });
              }
  ...
  genHttpServerConfig(key, cfg) {
    ...
    cfg.locations && Object.keys(cfg.locations).forEach((router) => {
      ...
      if (v.proxyPass) {
        ...
        if (v.locationParam) {
          _.merge(v, v.locationParam);
        }
        delete v.locationParam;
  ...
  stringifyLocation(router, obj, indent) {
    ...
    Object.keys(obj).forEach((key) =>{
      ...
      if (Array.isArray(v)) {
        v.forEach((value) => {
          info.push([indent + 4, `${key} ${wrapValue(value)}`]);
        });
      } else {
        info.push([indent + 4, `${key} ${wrapValue(v)}`]);
      }
    });
    info.push([indent + 2, '}']);
    return utils.stringifyInfo(info);
