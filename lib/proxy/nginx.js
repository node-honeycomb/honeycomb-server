...
class Nginx extends Proxy {
   ...
   *           timeout: 1000,
   *           check: 'HEAD / HTTP/1.0\r\n\r\n',
   *           expect: 'http_2xx http_3xx'
   *
   *
   *         upstreamCheck:
   *           type: 'normal',
   *           max_fails: 1,      // default 1
   *           fail_timeout: 1000 // if fail, wait xx ms
   *
   *
   */
  constructor(options) {
  ...
  initConfig() {
    ...
    let ast = NgConfigParser.parse(code, {loc: true});
    ...
    /*
    NginxConfFile.create(this.configPath, (err, conf) => {
      /* istanbul ignore if
      if (err) {
        err.message = 'loading nginx config error' + err.message;
        return this.emit('error', err);
      }
      let flag = false;
      let include = conf.nginx.http.include;

      // let uinfo = userInfo(process.getuid());

      /** 用户权限问题，改成 sock文件创建成可读可写 *
      /*
      if (!conf.nginx.user) {
        return log.error(FLAG_NGINX, 'nginx config should have `user` directive');
      } else if (conf.nginx.user._value) {
        let nginxUser = conf.nginx.user._value.split(/ +/);
        let nodeUser = uinfo.username + ' ' + uinfo.groupname;
        if (nginxUser.length === 1) {
          nginxUser.push(nginxUser[0]);
        }
        nginxUser = nginxUser.join(' ');
        if (nginxUser !== nodeUser) {
          log.error(FLAG_NGINX, `nginx user [${nginxUser}] !== node user [${nodeUser}], proxy have no permission`);
          log.error(FLAG_NGINX, 'please make sure both nginx processor and node processor are run with the same user');
          return;
        }
      }
      *

      /** websocket upgrade 配置， 需要一个map *
      /*
      if (!conf.nginx.http.map) {
        conf.nginx.http._add('map', '$http_upgrade $connection_upgrade');
        let map = conf.nginx.http.map;
        map._add('default', 'upgrade');
        map._add('""', 'close');
      } else {
        let flag = false;
        if (!Array.isArray(conf.nginx.http.map)) {
          conf.nginx.http.map = [conf.nginx.http.map];
        }
        conf.nginx.http.map.forEach((map) => {
          if (map._value === '$http_upgrade $connection_upgrade') {
            flag = true;
          }
        });
        if (!flag) {
          conf.nginx.http._add('map', '$http_upgrade $connection_upgrade');
          let map = conf.nginx.http.map[conf.nginx.http.map.length - 1];
          map._add('default', 'upgrade');
          map._add('""', 'close');
        }
      }
      ...
      /** 添加include指令
      if (!include) {
        conf.nginx.http.include = [];
      } else if (!Array.isArray(include)) {
        conf.nginx.http.include = [include];
      }
      conf.nginx.http.include.forEach((v) => {
        let tmp = v.toString();
        if (tmp.indexOf(this.includePath) > 0) {
          flag = true;
        }
      });
      if (flag) {
      ...
      conf.nginx.http._add('include', path.join(this.includePath, 'http/*.conf'));
      conf.nginx.http._add('include', path.join(this.includePath, 'stream/*.conf'));
      process.nextTick(() => {
        this.emit('ready');
      });
    });
  ...
  reload(callback) {
    ...
    child.exec(cmdCheck, (err, stdout, stderr) => {
      if (err) {
        ...
        callback && callback(new Error('nginx reload failed, config error'));
        return;
      }
      child.exec(cmdReload, (err, stdout, stderr) => {
        if (err) {
          log.error(FLAG_NGINX, 'nginx_reload_error', stderr.toString());
          if (!this.flagRollBack) {
            this.flagRollBack = true;
            this.rollback();
          }
  ...
  cleanErrorConfig() {
    let p;
    p = path.join(this.includePath, 'http_error');
    xfs.sync().rm(p);
    p = path.join(this.includePath, 'stream_error');
    xfs.sync().rm(p);
  }
  backupErrorConfig() {
    let httpError;
    ...
    let http;
    let stream;
    httpError = path.join(this.includePath, 'http_error');
    streamError = path.join(this.includePath, 'stream_error');
    if (!xfs.existsSync(httpError)) {
      http = path.join(this.includePath, 'http');
      xfs.existsSync(http) && xfs.renameSync(http, httpError);
    }
    if (!xfs.existsSync(streamError)) {
      stream = path.join(this.includePath, 'stream');
      xfs.existsSync(stream) && xfs.renameSync(stream, streamError);
    }
  }
}
