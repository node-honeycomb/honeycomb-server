...
class Nginx extends Proxy {
  ...
  backupErrorConfig() {
    let d = new Date().toLocaleString().replace(/ /g, '_');
    let errBackup = path.join(path.dirname(this.includePath), 'nginx_error_' + d);
    xfs.renameSync(this.includePath, errBackup);
